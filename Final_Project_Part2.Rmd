...
tq_get(c("MSFT","AAPL", "AMZN","FB","GOOG"), get = "stock.prices") %>% 
  head()
tq_get(c("MSFT","GOOG", "AMZN","FB","AAPL"), get = "stock.prices") %>% 
  group_by(symbol) %>% 
  tq_transmute(select = adjusted, 
               mutate_fun = periodReturn, 
               period = "monthly", 
               col_rename = "monthly_return") %>% 
  ggplot(aes(date, monthly_return, color = symbol)) + geom_line()+theme_tq()+ scale_color()
```
...
Industrials <- sp500 %>% 
  ...
  arrange(desc(weight)) %>% 
  slice(1:10) 
Industrials %>% select(symbol, company, weight, sector,shares_held,last.sale.price,market.cap,ipo.year) %>% head()
```
